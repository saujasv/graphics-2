...
Bomb::Bomb(glm::vec3 pos, glm::vec3 dir, float speed) {
    this->position = glm::vec3(pos);
    this->velocity = glm::vec3(dir.x * (speed), dir.y * (speed), dir.z * (speed));
    ...
    this->body = create3DObject(GL_TRIANGLES, 6 * N, body_vertex_buffer, COLOR_RED, GL_FILL);
    ...
    this->nose = create3DObject(GL_TRIANGLES, 6 * N, nose_vertex_buffer, COLOR_RED, GL_FILL);
}
